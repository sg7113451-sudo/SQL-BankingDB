...
select transactionid, amount, sum(amount) over(order by transactiondate asc) as ranning_total from transactions;
 select * from transactions;
  ...
  select * from transactions;
  select * from transactions where tranasactiontype in ('deposit','withdraw');
  select * from customers where customerid in (101,102,105);
  select * from accounts where accounttype in ('salary','savings');
  select * from transactions where tranasactiontype in ('payment','deposit');
  select * from customers where customerid in (103,104);
  select * from accounts where accountid in (201,202,205);
  select * from customers order by lastname asc;
  ...
  select * from transactions order by transactiondate limit 5;
